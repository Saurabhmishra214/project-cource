                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('social_media.add') }}" class="btn btn-primary">
                                    ...
                                    <tbody>
                                        @forelse($socialMedias as $socialMedia)
                                            <tr>
                                                ...
                                                <td>{{ $socialMedia->profile_url }}</td>
                                                <td>{{ $socialMedia->username }}</td>
                                                <td>{{ $socialMedia->account_link }}</td>
                                                <td>{{ $socialMedia->followers_count }}</td>
                                                <td class="text-end">
                                                    {{-- <a href="{{ route('digitalproduct.details', $product->id) }}" class="btn btn-sm btn-outline-primary me-1">
                                                        <i class="fa-solid fa-eye fs-6"></i>
                                                    </a> --}}
                                                    <a href="{{ route('social_media.edit', $socialMedia->id) }}" class="btn btn-sm btn-outline-secondary me-1">
                                                        <i class="fa-solid fa-pen-to-square fs-6"></i>
                                                    </a>
                                                    <form action="{{ route('social_media.destroy', $socialMedia->id) }}" method="POST" style="display:inline;">
                                                        @csrf
                                                        @method('DELETE')
